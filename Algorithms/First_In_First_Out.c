/*
 * Simulateur d'Ordonnancement de Processus - First In First Out
    ...
    while (finished < config->process_count) {
        ...
        for (int i = 0; i < config->process_count; i++) {
            ...
            if (p.arrival_time == time && !pcb[i].finished && !pcb[i].in_io) {
                ...
                log_print("[t=%d] Arrival: %s → ready queue\n", time, p.ID);
            }
        }
        ...
        while (node) {
            ...
            if (current_pcb) {
                ...
                if (current_pcb->io_remaining <= 0) {
                    ...
                    log_print("[t=%d] %s: I/O finished → ready queue\n", time, current_pcb->process.ID);
                    current_pcb->in_io = 0;
        ...
        if (ready.size > 0) {
            ...
            if (current_pcb) {
                ...
                if (start_io) {
                    ...
                    printf("[t=%d] %s → starts I/O (duration=%d)\n", time, p.ID,p.io_operations[current_pcb->io_index].duration);
                    log_print("[t=%d] %s → starts I/O (duration=%d)\n", time, p.ID,p.io_operations[current_pcb->io_index].duration);
                ...
                if (current_pcb) {
                    ...
                    printf("[t=%d] CPU → %s (executed=%d, remaining=%d)\n", time, current_pcb->process.ID,  current_pcb->executed_time, current_pcb->remaining_time);
                    log_print("[t=%d] CPU → %s (executed=%d, remaining=%d)\n", time, current_pcb->process.ID,  current_pcb->executed_time, current_pcb->remaining_time);
